...
const Formulas = () => {
  ...
  // Función para calcular la cantidad según el peso y porcentaje
  const calcularCantidad = (productoId) => {
    ...
    if (productoSeleccionado && peso) {
      ...
      switch (color) {
        case 'Negro':
          porcentaje = productoSeleccionado.porc_negro;
          break;
        default:
          porcentaje = 0;
      }

      return (peso * porcentaje).toFixed(2);
    }
  ...
  const handleProductoChange = (e, index) => {
    ...
    nuevasFilas[index].cantidad = calcularCantidad(nuevoValor);
  ...
  const handlePesoChange = (e) => {
    ...
    // Recalcular cantidades cuando cambia el peso
    const nuevasFilas = filas.map(fila => ({
      ...fila,
      cantidad: calcularCantidad(fila.producto),
    }));
  ...
  const handleColorChange = (e) => {
    ...
    // Recalcular cantidades cuando cambia el color
    const nuevasFilas = filas.map(fila => ({
      ...fila,
      cantidad: calcularCantidad(fila.producto),
    }));
